%% PRODUCTIVE AGENT WEALTH TAX
% Full Specified Life Cycle profile
% Full specified HGeterogeneous returns  R = [1.15 , 1.05, 1.1, 1.01 ]; 
...
% Uncertain Element: 
R = [1+0.15*(1-taur) , 1+0.05*(1-taur)]; 
Ny = numel(R); 
rTrMat = [0.8 , 0.2,; 0.5, 0.5]; 
...
for t = T:-1:1
    ...
    cons(NotFeas) = 1;
     ...
     if t == T 
       ...
       NotFeas = (cons>aToday.*(1-taua)+yvec(t));
     end 
...
title('Productive')       % of consumption. Note: 'ctMean(:)'
